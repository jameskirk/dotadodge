//package dotadodge.ui.directx;
//
//import static com.fourthskyinteractive.dx4j.d3d9.D3D9.D3DADAPTER_DEFAULT;
//import static com.fourthskyinteractive.dx4j.d3d9.D3D9.D3DCLEAR_TARGET;
//import static com.fourthskyinteractive.dx4j.d3d9.D3D9.D3DCREATE_SOFTWARE_VERTEXPROCESSING;
//import static com.fourthskyinteractive.dx4j.d3d9.D3D9.D3DFVF_DIFFUSE;
//import static com.fourthskyinteractive.dx4j.d3d9.D3D9.D3DFVF_XYZRHW;
...
//import static com.fourthskyinteractive.dx4j.d3d9.D3D9.D3DDEVTYPE.D3DDEVTYPE_HAL;
...
//	final IDirect3DDevice9 device = d3d9.CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL,
//		new HWND(JAWTUtils.getNativePeerHandle(frame)), D3DCREATE_SOFTWARE_VERTEXPROCESSING, d3dpp);
//
//	// Creating vertex buffer
//	int primitiveCount = 2;
//	final IDirect3DVertexBuffer9 vertexBuffer = device.CreateVertexBuffer((int) (primitiveCount * 3 * sizeOf(D3DVERTEX.class)), 0,
//		D3DFVF_XYZRHW | D3DFVF_DIFFUSE, D3DPOOL.D3DPOOL_DEFAULT);
//
//	// Copying data
//	Pointer<Pointer<?>> lockedMem = allocatePointer();
//	if (vertexBuffer.Lock(0, (int) (primitiveCount * 3 * sizeOf(D3DVERTEX.class)), lockedMem, 0) == 0) {
//
//	    // Direct attribuition
...
//	    pVertices.set(3, new D3DVERTEX().x(0.0f).y(0.0f).z(0.0f).rhw(1.0f).color(0x00ff00));
//	    pVertices.set(4, new D3DVERTEX().x(150.0f).y(0.0f).z(0.0f).rhw(1.0f).color(0x0000ff));
//	    pVertices.set(5, new D3DVERTEX().x(0.0f).y(150.0f).z(0.0f).rhw(1.0f).color(0xff0000));
//
//	    // Alternative way: copy data
//	    // Pointer<D3DVERTEX> vertices = allocateArray(D3DVERTEX.class, 3);
//	    // vertices.set(0, new
//	    // D3DVERTEX().x(50.0f).y(50.0f).z(0.0f).rhw(1.0f).color(0x00ff00));
//	    // vertices.set(1, new
//	    // D3DVERTEX().x(250.0f).y(50.0f).z(0.0f).rhw(1.0f).color(0x0000ff));
//	    // vertices.set(2, new
//	    // D3DVERTEX().x(50.0f).y(250.0f).z(0.0f).rhw(1.0f).color(0xff0000));
...
//			    float x = lockedMem.get().as(D3DVERTEX.class).get(0).x();
//			    x = x + 10;
//			    lockedMem.get().as(D3DVERTEX.class).get(0).x(x);
//			    device.SetStreamSource(0, pointerTo(vertexBuffer), 0, (int) sizeOf(D3DVERTEX.class));
//			    device.SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
//			    device.DrawPrimitive(D3DPRIMITIVETYPE.D3DPT_TRIANGLELIST, 0, primitiveCount);
//			    device.EndScene();
//			}
//
